...
{{ Form::open(array('url' => 'exercisesets')) }}
    ...
    <div ng-app="ngApp" ng-controller="ngCtrl" class="form-group">
                ...
                <tr ng-repeat="setexercise in setexercises"  id="setexercise<%setexercise.id%>">
                    ...
                    <select name="type_id" ng-model="setexercise.type_id" class="form-control exercisetype_select">
                       @foreach($exercisetypes as $key => $value)
                            <option  ng-selected="{{$value->id}} == <%id%>" value="{{$value->id}}">{{$value->name}}</option>
                        @endforeach
                    </select>
                    ...
                    <td><button type="button" data-itemId="<%$index%>" ng-click="deleteItem($event)" class="btn btn-danger">Delete</button></td>
                </tr>
                <tr>
                    <td></td>
                    <td></td>
    ...
    <div class="form-group">
        {{ Form::hidden('id', $exercisesetcount + 1, array('class' => 'form-control')) }}
    </div>

    {{ Form::submit('Create the exerciseset!', array('class' => 'btn btn-primary')) }}
